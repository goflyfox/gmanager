...
// 通过id获取实体
func (s *logSvc) GetById(ctx context.Context, id int64) (*model.Log, error) {
	output := &model.Log{}
	if id <= 0 {
		glog.Error(" get id error")
		return new(model.Log), errors.New("参数不合法")
	}

	err := dao.Log.Ctx(ctx).WherePri(id).Scan(&output)
	if err != nil {
		return nil, err
	}
	return output, nil
...
// 根据条件获取实体
func (s *logSvc) GetOne(ctx context.Context, form *base.BaseForm) (*model.Log, error) {
	output := &model.Log{}
	...
	err := dao.Log.Ctx(ctx).Where(where, params).Scan(&output)
...
// 删除实体
func (s *logSvc) Delete(ctx context.Context, id int64, userId int) error {
	if id <= 0 {
		glog.Error("delete id error")
		return errors.New("参数不合法")
	}

	_, err := dao.Log.Ctx(ctx).WherePri(gconv.Int(id)).Delete()
	if err != nil {
		return err
	}
...
// 保存实体
func (s *logSvc) Save(ctx context.Context, request *LogReq) (int64, error) {
	entity := (*model.Log)(nil)
	err := gconv.Struct(request.Log, &entity)
	if err != nil {
	...
	if entity.Id <= 0 {
		...
		r, err := dao.Log.Ctx(ctx).Insert(entity)
		if err != nil {
			return 0, err
		}
		// 回写主键
		...
		r, err := dao.Log.Ctx(ctx).OmitEmpty().Where(" id = ?", entity.Id).Update(entity)
		if err != nil {
			return 0, err
		}
...
// 列表数据查询
func (s *logSvc) List(ctx context.Context, form *base.BaseForm) (list []*model.Log, err error) {
	where := " 1 = 1 "
	var params []interface{}
	...
	err = dao.Log.Ctx(ctx).Order(form.OrderBy).Where(where, params).Scan(&list)
	return
}

// 分页查询
func (s *logSvc) Page(ctx context.Context, form *base.BaseForm) (list []*model.Log, err error) {
	...
	num, err := dao.Log.Ctx(ctx).As("t").FindCount(where, params)
	form.TotalSize = num
	...
	dbModel := dao.Log.Ctx(ctx).As("t").Fields(s.Columns() + ",su1.real_name as updateName,su2.real_name as createName")
