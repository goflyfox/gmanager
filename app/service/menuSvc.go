...
// 通过id获取实体
func (s *menuSvc) GetById(ctx context.Context, id int64) (*model.Menu, error) {
	output := &model.Menu{}
	if id <= 0 {
		glog.Error(" get id error")
		return new(model.Menu), errors.New("参数不合法")
	}

	err := dao.Menu.Ctx(ctx).WherePri(id).Scan(&output)
	if err != nil {
		return nil, err
	}
	return output, nil
...
// 根据条件获取实体
func (s *menuSvc) GetOne(ctx context.Context, form *base.BaseForm) (*model.Menu, error) {
	output := &model.Menu{}
	...
	err := dao.Menu.Ctx(ctx).Where(where, params).Scan(&output)
...
// 删除实体
func (s *menuSvc) Delete(ctx context.Context, id int64, userId int) error {
	if id <= 0 {
		glog.Error("delete id error")
		return errors.New("参数不合法")
	}

	_, err := dao.Menu.Ctx(ctx).WherePri(gconv.Int(id)).Delete()
	if err != nil {
		return err
	}
...
// 保存实体
func (s *menuSvc) Save(ctx context.Context, request *MenuReq) (int64, error) {
	entity := (*model.Menu)(nil)
	err := gconv.Struct(request.Menu, &entity)
	if err != nil {
	...
	if entity.Id <= 0 {
		...
		r, err := dao.Menu.Ctx(ctx).Insert(entity)
		if err != nil {
			return 0, err
		}
		// 回写主键
		...
		r, err := dao.Menu.Ctx(ctx).OmitEmpty().Where(" id = ?", entity.Id).Update(entity)
		if err != nil {
			return 0, err
		}
...
// 列表数据查询
func (s *menuSvc) List(ctx context.Context, form *base.BaseForm) (list []*model.Menu, err error) {
	where := " 1 = 1 "
	var params []interface{}
	...
	err = dao.Menu.Ctx(ctx).Order(form.OrderBy).Where(where, params).Scan(&list)
	return
}

// 分页查询
func (s *menuSvc) Page(ctx context.Context, form *base.BaseForm) (list []*model.Menu, err error) {
	...
	num, err := dao.Menu.Ctx(ctx).As("t").FindCount(where, params)
	form.TotalSize = num
	...
	dbModel := dao.Menu.Ctx(ctx).As("t").Fields(s.Columns() + ",su1.real_name as updateName,su2.real_name as createName")
