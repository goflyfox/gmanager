      ...
      <el-form ref="menuFormRef" :model="formData" :rules="rules" label-width="100px">
        ...
        <el-form-item v-if="formData.type == MenuTypeEnum.MENU">
          ...
          <div v-if="!formData.paramList || formData.paramList.length === 0">
            <el-button type="success" plain @click="formData.paramList = [{ key: '', value: '' }]">
              添加路由参数
            </el-button>
          </div>
          ...
          <div v-else>
            <div v-for="(item, index) in formData.paramList" :key="index">
              ...
              <el-icon
                v-if="formData.paramList.indexOf(item) === formData.paramList.length - 1"
                class="ml-2 cursor-pointer color-[var(--el-color-success)]"
                style="vertical-align: -0.15em"
                @click="formData.paramList.push({ key: '', value: '' })"
              >
                <CirclePlusFilled />
              </el-icon>
              <el-icon
                ...
                style="vertical-align: -0.15em"
                @click="formData.paramList.splice(formData.paramList.indexOf(item), 1)"
              >
                <DeleteFilled />
              </el-icon>
            </div>
        ...
        <el-form-item v-if="formData.type !== MenuTypeEnum.BUTTON" prop="enable" label="显示状态">
...
function handleRowClick(row: MenuVO) {
  selectedMenuId.value = String(row.id);
}
  ...
  menuFormRef.value.validate((isValid: boolean) => {
    if (isValid) {
      ...
      if (menuId) {
        ...
        MenuAPI.save(menuId, formData.value).then(() => {
          ElMessage.success("修改成功");
          handleCloseDialog();
          handleQuery();
        });
      } else {
        MenuAPI.save(0, formData.value).then(() => {
